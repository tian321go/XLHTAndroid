...
import com.axeac.android.sdk.ui.chart.MixedChart;
...
public class Mixed extends Component {
	...
	private boolean havePercent = false;

	public Mixed(Activity ctx) {
		super(ctx);
	...
	public void addData(String data) {
		...
		if (items[3].endsWith("%")){
			havePercent = true;
			if (items.length >= 5 && CommonUtil.isFloat(items[3].substring(0,items[3].length()-1))) {
				String val = items[3].trim();
				if (CommonUtil.isFloat(val.substring(0,val.length()-1))) {
	...
	public void execute() {
		...
		mChart.setHavePercent(havePercent);
	}
